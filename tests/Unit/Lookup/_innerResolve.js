    ...
    ipVersions.forEach(ipVersion => {
        it(`must correct return cached value for IPv${ipVersion}`, done => {
            ...
            setImmediate(() => {
                ...
                assert.isTrue(
                    callbackSpy.calledWithExactly(null, cachedAddress)
                );
    ...
    ipVersions.forEach(ipVersion => {
        it(`must correct add callback for the found task for IPv${ipVersion}`, done => {
            ...
            setImmediate(() => {
                ...
                assert.isTrue(
                    taskAddResolvedCallbackSpy.calledWithExactly(callbackSpy)
                );
    ...
    ipVersions.forEach(ipVersion => {
        it(`must correct create task, add callback to it, run for IPv${ipVersion} and correct handle events`, done => {
            ...
            setImmediate(() => {
                ...
                assert.strictEqual(
                    addressCacheSetSpy.getCall(0).args[1],
                    expectedAddresses
                );
                ...
                assert.strictEqual(
                    resolveTaskOnSpy.getCall(0).args[0],
                    'addresses'
                );
                assert.instanceOf(
                    resolveTaskOnSpy.getCall(0).args[1],
                    Function
                );

                assert.strictEqual(resolveTaskOnSpy.getCall(1).args[0], 'done');
                assert.instanceOf(
                    resolveTaskOnSpy.getCall(1).args[1],
                    Function
                );
