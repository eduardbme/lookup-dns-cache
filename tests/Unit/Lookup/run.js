...
describe('Unit: Lookup::run', () => {
    ...
    invalidSetOfOptions.forEach(invalidOptions => {
        it(`must throw exception, cuz family option is invalid ${JSON.stringify(
            invalidOptions
        )}`, () => {
            const cb = () => {};
    ...
    const setOfOptions = [{ family: 4 }, { family: 6 }];

    setOfOptions.forEach(options => {
        it(`must correct call _resolve method with appropriate params and options ${JSON.stringify(
            options
        )}`, () => {
            const cb = () => {};
