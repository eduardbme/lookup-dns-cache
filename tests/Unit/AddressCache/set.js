...
describe('Unit: AddressCache::set', () => {
    it('must correct set addressed for particular key', () => {
        const setOfAddresses1 = [{ address: '1.2.3.4' }];

        const setOfAddresses2 = [{ address: '5.6.7.8' }];
        ...
        assert.deepEqual(
            addressesCache._cache.get(addresses.INET_HOST1),
            setOfAddresses1
        );
        assert.deepEqual(
            addressesCache._cache.get(addresses.INET_HOST2),
            setOfAddresses2
        );
    });
});
